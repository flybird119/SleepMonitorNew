...
public class BTClient extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.btmain);   //设置画面为主画面 btmain.xml
    		...
    		while(true){
    			try{
    				...
    				while(true){
    					...
    					smsg+=s;   //写入接收缓存
    					if(is.available()==0)break;  //短时间没有数据才跳处进行显示
    				}
    				//发送显示消息，进行显示刷新
  ...
  /*  @Override
	...
	private void Save() {
		//显示对话框输入文件名
		LayoutInflater factory = LayoutInflater.from(BTClient.this);  //图层模版生成器句柄
		final View DialogView =  factory.inflate(R.layout.sname, null);  //用sname.xml生成试图模版
		new AlertDialog.Builder(BTClient.this)
								.setTitle("文件名")
								.setView(DialogView)   //设置视图模版
									...
									public void onClick(DialogInterface dialog, int whichButton){
										...
										try{
											if(Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)){  //如果SD卡已准备好
												...
												File saveFile =new File(BuildDir, filename);  //新建文件句柄，如已存在仍然新建文档
								...
								.setNegativeButton("取消",   //取消按键响应函数，直接退出对话框不作任何处理
